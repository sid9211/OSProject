...
envid2env(envid_t envid, struct Env **env_store, bool checkperm)
{
	...
	//cprintf ("\n\nEnv id %d \n  Env status %d\n Envid envid %d\n checkperm %d \n",e->env_id,e->env_status,envid,checkperm);
...
env_setup_vm(struct Env *e)
{
	...
	pde_t *pdp1 = (pde_t*)page2kva(p);
	pml4e_t *pml4e2 = boot_pml4e + PML4(UTOP);
	
	if(!(*pml4e2 & PTE_P))
		return -E_NO_MEM;
	
	pdpe_t *pdpe2 = KADDR(PTE_ADDR(*pml4e2));
	pdpe2 += PDPE(UTOP);
	
	if(!(*pdpe2 & PTE_P))
		return -E_NO_MEM;
	pde_t *pdp2 = KADDR(PTE_ADDR(*pdpe2));
	memcpy(pdp1, pdp2, PGSIZE);
	
	if(!(p = page_alloc(ALLOC_ZERO)))
		return -E_NO_MEM;
	p->pp_ref++;
	
	pdpe_t *pdpe1 = (pdpe_t*)page2kva(p);
	pdpe1[PDPE(UTOP)] = PADDR(pdp1) | PTE_USER;
	...
	e->env_pml4e[PML4(UTOP)] = PADDR(pdpe1) | PTE_USER;
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for(;a<b; a+=PGSIZE)
	{
		pte=pml4e_walk(e->env_pml4e,(void *)(a),1);
			//cprintf("Calling page alloc %dth time\n", ++i);
			p=page_alloc(0);
			if(p==NULL)
				panic(" Can't page_alloc \n ");
			if((page_insert(e->env_pml4e,p,(void *) a,PTE_U|PTE_W))== -E_NO_MEM)
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	if( (elf!=NULL) &&  elf -> e_magic != ELF_MAGIC)
	...
	e->env_tf.tf_rip = (uintptr_t)(elf->e_entry);
